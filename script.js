  ...
  videocont.addEventListener("click", function () {
    if (flag == 0) {
      ...
      document.querySelector(
        "#playbtn"
      ).innerHTML = `<i class="ri-pause-mini-line" id="playicon"></i>`;
      flag = 1;
    } else {
      video.pause();
      ...
      document.querySelector(
        "#playbtn"
      ).innerHTML = `<i class="ri-play-fill" id="playicon"></i>`;
      flag = 0;
    }
  });
}
...
function sherygoeyeffect() {
  Shery.imageEffect(".img-div", {
    ...
    config: {
      ...
      aspect: { value: 0.8000140785583556 },
      ignoreShapeAspect: { value: true },
      shapePosition: { value: { x: 0, y: -0.010752688172043001 } },
      ...
      uColor: { value: false },
      uSpeed: { value: 0.3, range: [0.1, 1], rangep: [1, 10] },
      uAmplitude: { value: 1.98, range: [0, 5] },
      uFrequency: { value: 5.24, range: [0, 10] },
    },
...
function projectheading() {
  ...
  projheading.forEach(function (elem) {
    elem.addEventListener("mouseenter", function (dets) {
      var projanime = $(dets.target).textillate({
        minDisplayTime: 0.005,
    ...
    elem.addEventListener("mouseleave", function (dets2) {
      var projanime2 = $(dets2.target).textillate({
